#ifndef OPENSIM_ACTIVATION_POINT_ACTUATOR_H
#define OPENSIM_ACTIVATION_POINT_ACTUATOR_H
/* -------------------------------------------------------------------------- *
 *                  OpenSim:  ActivationPointActuator.h                  *
 * -------------------------------------------------------------------------- *
...
/**
Similar to PointActuator (simply produces a generalized force) but
...
@endverbatim
 */
class OSIMPLUGIN_API ActivationPointActuator
        : public PointActuator{
    OpenSim_DECLARE_CONCRETE_OBJECT(ActivationPointActuator,
        PointActuator);
public:
    ...
    OpenSim_DECLARE_OUTPUT(activation, double,
        getActivation, SimTK::Stage::Dynamics);

    ActivationPointActuator() {
        constructProperties();
    }

    /// Provide the coordinate name.
    explicit ActivationPointActuator(const std::string& bodyName)
            : ActivationPointActuator() {
        if (!bodyName.empty()) {
            set_body(bodyName);
        }
...
#endif // OPENSIM_ACTIVATION_POINT_ACTUATOR_H