...
// Add the reporters and controllers to the model and saves a dummy copy of it
...
// The model reference is not const as we need to clear the reporter tables
  ...
  // get exponential increase weight 
...
inline std::vector<double> dVector(const std::vector<double> &vec){
...
inline std::vector<double> dVector(SimTK::VectorView &vec){
  std::vector<double> outVec(vec.size());
  std::adjacent_difference(vec.begin(), vec.end(), outVec.begin());
  outVec[0] = 0;
  return outVec;
