...
// Adds controller to the human model and saves a copy
...
std::vector<double> runSimulation(OpenSim::Model &osimModel, const ParameterizationType &parameterization, 
  ...
  try{
    ...
    //std::uniform_int_distribution<int> uniformNoise(0, 20);
    //const double noise =  (double)uniformNoise(generator)/100;
    ...
    std::for_each(costs.begin(), costs.end(), [](double cost){std::cout << cost << ", ";});
    std::cout << std::endl;
  }
...
void progressFunc(const CMAParameters<GenoPhenoType> &cmaparams, const CMASolutions &cmasols, const std::string logFolder){
    ...
    std::vector<double> vecOutMean;
    vecOutMean.push_back(cmasols.niter());
    vecOutMean.push_back(0.0);
    Eigen::VectorXd meanParams = cmaparams.get_gp().pheno(cmasols.xmean());
    vecOutMean.insert(vecOutMean.end(), meanParams.data(), meanParams.data() + meanParams.rows()*meanParams.cols());
    writeVector(vecOutMean, logFolder+logMeanFileName);
    
    // Writing log
    if (cmasols.best_candidate().get_fvalue() == cmasols.get_best_seen_candidate().get_fvalue()){
        std::vector<double> vecOut;
        vecOut.push_back(cmasols.niter());
        vecOut.push_back(cmasols.best_candidate().get_fvalue());
        Eigen::VectorXd bestparameters = cmaparams.get_gp().pheno(cmasols.best_candidate().get_x_dvec());
        vecOut.insert(vecOut.end(), bestparameters.data(), bestparameters.data() + bestparameters.rows() * bestparameters.cols());
        writeVector(vecOut, logFolder+logBestFileName);
    }
};
