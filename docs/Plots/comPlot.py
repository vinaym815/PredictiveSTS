...
startTime, endTime = computeMotionTimeRange(("time", "hip_flexion"), (startThreshold, endThreshold), 
                            motionFileName, 8, simStartEndnPointMovingAvg)    
...
motion_file = file(jointNames, motionFileName, 8)
...
colorbar = fig.colorbar(line, ax=axs[0])

ylim = [-0.2, 1.0]
axs[1].plot(motionXAxis, com_vel_x, label=r"$\dot{COM_x}$")
axs[1].plot(motionXAxis, com_vel_y, label=r"$\dot{COM_y}$")
axs[1].plot([e1,e1], ylim, linestyle='-.', color="k", alpha=0.5)
axs[1].plot([e2,e2], ylim, linestyle='-.', color="k", alpha=0.5)

axs[0].axis('equal')
colorbar.set_label(r"$\%STS$")
axs[0].set_xlabel(r"$COM_x(m)$")
axs[0].set_ylabel(r"$COM_y(m)$")
axs[1].set_xlabel(r"$\%STS$")
axs[1].set_ylabel(r"$Velocity (m/s)$")
axs[1].legend(prop={'size': 10})
#axs.set_ylim(0.65, 0.96)
axs[0].set_xlim(-0.2, 0.1)
